...
<CFFORM ACTION="IMPORT_MASTER_DBF2.cfm" METHOD="POST" ENABLECAB="Yes">
				...
				<CFQUERY Name="AddNewClient" datasource="#Client.database#" DBType="ODBC">
					...
					<CFTRY>
						<CFQUERY Name="AddClientDetails" datasource="#Client.database#" DBType="ODBC">
					        ...
					        Values
							(
									  '#UCase(Trim(MSACCODE))#', '#UCase(Trim(MSADDRL1))#~#UCase(Trim(MSADDRL2))#~#UCase(Trim(MSADDRL3))# #UCase(Trim(MSADDRL4))# #UCase(Trim(MSADDRL5))#', '#mstelno#'
							)
						</CFQUERY>
  ...
  <CFIF IsDefined("cmdImportBrokerage")>
	...
	<CFOUTPUT QUERY="GetBrk">
  	...
  	<CFIF	IsDefined("cmdImportClientBrk")>
		<CFQUERY NAME="GetBrk" DBTYPE="Dynamic" connectstring="Driver={Microsoft dBASE Driver (*.dbf)}; DriverID=277; "DEBUG>
			SELECT		* 						
		...
		<CFQUERY NAME="DeleteData" datasource="#Client.database#" DBTYPE="ODBC">
			...
			WHERE	COMPANY_CODE	=	'#CmbCoList#'
 		</CFQUERY>
		
	<CFOUTPUT QUERY="GetBrk">

		<CFTRY>
			<CFQUERY NAME="ValidateClient" datasource="#Client.database#" DBTYPE="ODBC"DEBUG>
				Select	Client_ID
				From	
						Client_Master
				Where
			...
			<CFQUERY NAME="ValidateModule" datasource="#Client.database#" DBTYPE="ODBC"DEBUG>
				Select	Module_No
				From	
						Brokerage_Master
				Where 
			...
			<CFIF	ValidateClient.RecordCount GT 0
			  And	ValidateModule.RecordCount GT 0>
	 			...
	 			<CFQUERY Name="AddNewClientBrkApply" datasource="#Client.database#" DBType="ODBC"DEBUG>
	  ...
	  <CFQUERY NAME="InsertBlank" datasource="#Client.database#" DBTYPE="ODBC">
			...
			Select 
					'#UCase(Trim(CmbCoList))#', Client_ID, '2005-04-01', Null, Null, Null,
					 3, 3, 'Y', 'Y', 'Y', 'Y'
			From
					Client_Master
			Where
