...
<CFSET Trade_Date = "#txtOrderDate#"> --->
	...
	<CFLOOP QUERY="GetClient">
		...
		<cfset RecordType = "2">
		<cfset Total_Number_Of_CNs = ContData.recordcount>
		<CFFILE ACTION="Write" FILE="#File_Name#" OUTPUT="{}"ADDNEWLINE="Yes">   <!--- {#Unique_Cl_ID1#} Changed for Eureka--->
		...
		<CFLOOP QUERY="ContData">
			...
			<cfinclude template="STP_Buy_Sale_Logic.cfm">
				...
				<cfset CNDate = "#DateFormat(trade_date,'YYYYMMDD')##TimeFormat(trade_dateTIME,'HHMMSS')#">
				<cfset Client_Exchange_Code = "#Client_Sebi_RegnNo1#">
				<cfset Custodian_Code = UNIQUECLCODE>
				<cfset Trade_Date7 = DateFormat(trade_date,'YYYYMMDD')> 
				...
				<cfset Settlement_Date =  DateFormat(SetlDate,'YYYYMMDD')>
				...
				<CFFILE ACTION="APPEND" FILE="#File_Name#" OUTPUT="#fileConvertString(BatchNumber, 7, 'R', '0')#15#fileConvertString('', 6, 'L', ' ')##fileConvertString(LineNumber, 5, 'R', '0')##fileConvertString(CNNO, 16, 'L', ' ')##fileConvertString(CNDate, 14, 'R', '0')##fileConvertString(Broker_Sebi_RegnNo, 12, 'L', ' ')##fileConvertString('', 4, 'L', ' ')##fileConvertString(Client_Exchange_Code, 16, 'L', ' ')##fileConvertString(Custodian_Code, 8, 'L', ' ')##fileConvertString('', 2, 'L', ' ')##fileConvertString(UCase(ISIN), 12, 'L', ' ')##fileConvertString(Trade_Date7, 8, 'R', '0')##fileConvertString(Market_Type, 2, 'L', ' ')##fileConvertString(SetlNO, 7, 'L', ' ')##fileConvertString(Trade_Flag, 1, 'L', ' ')##fileConvertString(Settlement_Type, 2, 'L', ' ')##fileConvertString(Settlement_Date, 8, 'R', '0')##fileConvertString('', 2, 'L', ' ')##fileConvertString(Avg_Rate, 15, 'R', '0')##fileConvertString(Total_Qty, 15, 'R', '0')##fileConvertString(Brokerage, 15, 'R', '0')##
				fileConvertString(Service_Tax, 15, 'R', '0')##fileConvertString(Net_Value, 15, 'R', '0')##fileConvertString(No_Of_Trade, 5, 'R', '0')##fileConvertString(Cust_Sebi_Reg_No, 12, 'L', ' ')##fileConvertString(Brokerage_Rate, 15, 'R', '0')##fileConvertString(Broker_Contact_Detail, 140, 'L', ' ')##fileConvertString(Order_Date, 8, 'R', ' ')##fileConvertString(Order_Time, 6, 'R', ' ')##fileConvertString(MAPIN_Id_Exchange, 9, 'R', '0')##fileConvertString(STT, 15, 'R', '0')#" ADDNEWLINE="Yes">
			...
			<cfif OptContractInf EQ "SUMMARY">
 			...
 			<cfelse>
				...
				<cfloop query="CONTRACTDETAILQUERY">
					...
					<Cfset TRADE_TIME  = TimeFormat(TRADE_dateTIME,'hhmmss')>
					<!--- ***************** --->
					<CFFILE ACTION="APPEND" FILE="#File_Name#" OUTPUT="#fileConvertString(BatchNumber, 7, 'R', '0')#13#fileConvertString(LineNumber, 5, 'R', '0')##fileConvertString('', 2, 'L', ' ')##fileConvertString(Broker_Sebi_RegnNo, 12, 'L', ' ')##fileConvertString(CNNO, 16, 'R', '0')##fileConvertString(ORDER_NO, 16, 'R', '0')##fileConvertString(Trade_No, 16, 'R', '0')##fileConvertString(Trade_Time, 6, 'R', '0')##fileConvertString(DETAILQTY, 15, 'R', '0')##fileConvertString(RATE7, 15, 'R', '0')#" ADDNEWLINE="Yes">
 				</cfloop>		
			</cfif>	
		</CFLOOP>
