...
<cfif "#DateFormat(now(),'yyyymmdd')#" gte '20130701'>
	<cfinclude template="McxgenerateUCINEW3.cfm">
	<cfabort>
</cfif>

<!--- <cfquery name="SETDATADATA" datasource="#CLIENT.DATABASE#">
	SELECT CASE WHEN CONVERT(DATETIME,CONVERT(VARCHAR(10),GETDATE(),103),103) > '2012-01-14' THEN 1 ELSE 0 END AS FIG
</cfquery>

<CFIF #SETDATADATA.FIG# EQ 1>
	<CFINCLUDE TEMPLATE="generateUCIMCXNEW.cfm">
	<cfabort>
</CFIF> --->
...
<CFIF getsystemsettingsvalue.UCCEXPORT_COMPULSORY EQ "Y">
	<CFINCLUDE TEMPLATE="generateUCIMCXCOMP.cfm">
	<cfabort>
</CFIF>
 --->	
...
<CFQUERY NAME="GetClientDetails" datasource="#Client.database#">
	SELECT DISTINCT  A.CLIENT_ID, pan_name CLIENT_NAME,B.MARKET,A.COMPANY_CODE,A.REGISTRATION_DATE,A.LAST_MODIFIED_DATE,
		...
		REL_WITH_INTRODUCER,INTRODUCER_ACC_CODE ,Registration_Number,Registration_Authority,Place_Of_Reg,Convert(Varchar(11),Date_of_Reg,106)  Date_of_Reg,
			...
			,UpdationFlag,RelationShip,TypeOfFacility,CLIENT_ID_MAIL EMAILID,MOBILE_NO,CIN
		...
		<CFIF IsDefined('frdate') and IsDefined('todate')>
			AND (
				...
				OR  CONVERT(DATETIME, convert(varchar(10),A.Agreement_Date, 103), 103) BETWEEN CONVERT(DATETIME,'#frdate#',103) 
				and CONVERT(DATETIME,'#todate#',103) 
				)
		</CFIF>
		<Cfif len(chkval) gt 0>
...
<CFLOOP QUERY="getClientDetails">
	<CFQUERY NAME="GetclientDpDetails" datasource="#Client.database#">
		...
		AND ISNULL(DEFAULT_NBFC,'') NOT IN ('SPOTY','SPOTPY')	
	...
	<CFIF  len(GetClientDetails.PAN_NO) eq 10>				
				...
				<CFSET LASTNAME = "#left(TRIM(GetClientDetails.LAST_NAME),100)#">
					...
					<CFSET MIDDLENAME = "#left(TRIM(GetClientDetails.MIDDLE_NAME),100)#">
				...
				<CFIF GetClientDetails.CATEGORY EQ 'I' OR GetClientDetails.CATEGORY EQ 'SP'>
					<CFSET CATEGORY1				=		"01">
				...
				<CFELSEIF 	GetClientDetails.CATEGORY EQ 'PMS'>
					<CFSET CATEGORY1				=		"08">
				<CFELSEIF 	GetClientDetails.CATEGORY EQ 'NBFC'>
					<CFSET CATEGORY1				=		"09">	
				<CFELSEIF 	GetClientDetails.CATEGORY EQ 'CO' or GetClientDetails.CATEGORY EQ 'PVTLTD' or GetClientDetails.CATEGORY EQ 'LTDCO' >
					<CFSET CATEGORY1				=		"13">	
				...
				<CFSET DP_ID1					=	"#LEFT(TRIM(GetclientDpDetails.DP_ID),8)#">
				<CFSET DEPOSITORY1				=	"#LEFT(TRIM(GetclientDpDetails.DEPOSITORY),4)#">
				<CFSET CLIENT_DP_CODE1 			= 	"#RIGHT(TRIM(GetclientDpDetails.CLIENT_DP_CODE),8)#">
				<CFSET DP_NAME1 				= 	"#LEFT(TRIM(GetclientDpDetails.DP_NAME),25)#">
				...
				<CFIF GetClientDetails.CATEGORY NEQ 'I' AND GetClientDetails.CATEGORY NEQ 'SP' AND GetClientDetails.CATEGORY NEQ 'NRI' AND REGISTRATION_NO NEQ "" AND REGISTRATION_AUTHORITY NEQ "" AND REGISTRATION_PLACE NEQ "" AND REGISTRATION_DATE1 NEQ "">
				...
				<CFSET FINALSTRING	   = "#FINALSTRING##LEFT(INTRODUCER_NAME,60)#,#LEFT(INTRODUCER_ACC_CODE,10)#,#LEFT(RELWITHINT,25)#,N,,1">
				<cfif UpdationFlag eq "Y">
					...
					<cfelseif TypeOfFacility EQ "2">
						<CFSET MOBILE_NO1 = "">
						<CFSET EMAILID1 = "#gettoken(Trim(GetClientDetails.EMAILID),1,';')#">
				...
				<cFIF UpdationFlag EQ 'y'>
						<CFSET FINALSTRING	   = "#FINALSTRING#,#VAL(TypeOfFacility)#,#LEFT(MOBILE_NO1,20)#,#LEFT(EMAILID1,50)#,#RelationShip#">
				<cfelse>
						<CFSET FINALSTRING	   = "#FINALSTRING#,0,#LEFT(MOBILE_NO1,20)#,#LEFT(EMAILID1,50)#,#RelationShip#">
				</cfif>		
					<cfif CATEGORY1 eq "13">
						<CFSET FINALSTRING	   = "#FINALSTRING#,#LEFT(CIN,25)#,">
					<cfelse>	
						<CFSET FINALSTRING	   = "#FINALSTRING#,,">
					</cfif>
					<CFSET FINALSTRING = "#FINALSTRING#,E#CHR(10)#">
...
<CFSET A = EODProcess("#COCD# UCI File Generate",'#frdate#','#Todate#','#COCD#','','','')>	
