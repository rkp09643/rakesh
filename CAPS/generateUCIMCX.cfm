...
<BODY  bgcolor="#EDECFF" TOPMARGIN="0">
...
<CFPARAM NAME="CmbActOption" DEFAULT="">
...
<CFIF GetClientDetails.recordcount eq 0 AND market1 NEQ 'All'>
	<SCRIPT>
		alert("No Data Found");
	</SCRIPT>
	<CFABORT>
...
<CFLOOP QUERY="getClientDetails">
	<CFQUERY NAME="GetclientDpDetails" datasource="#Client.database#">
		...
		AND ISNULL(DEFAULT_NBFC,'') IN ('SPOTY','SPOTPY','Com','Com_D','Com_O','Com_O_D','SptCom','SptCom_D','SptCom_D_O')			
...
<!--- TO GIVE UPDATE --->
				...
				<CFIF COCD EQ "MCX">
					...
					<cfif "#DateFormat(now(),'yyyymmdd')#" gte '20130201'>
							...
							<cfelse>
								<CFSET FINALSTRING	   = "#FINALSTRING##INTRODUCER_NAME#,#INTRODUCER_ACC_CODE#,#RELWITHINT#,N,,2,#TypeOfFacility1#,#MOBILE_NO1#,#EMAILID1#,,,,E">	
							</cfif>
					<CFELSE>
						<CFSET FINALSTRING	   = "#FINALSTRING##INTRODUCER_NAME#,#INTRODUCER_ACC_CODE#,#RELWITHINT#,N,,1,#TypeOfFacility1#,#MOBILE_NO1#,#EMAILID1#,,E">	
					</cfif>	
				...
				<cfelse>
					<cfset FINALSTRINGLOG = '#FINALSTRINGLOG#For #CLIENT_ID# Email ID is Missing#chr(10)#'>
				</cfif>
...
<CFIF GetClientDetails.recordcount gt 0>
	...
	<CFFILE ACTION="APPEND" FILE="#FILENAMELOG#" OUTPUT="#cocd# #FINALSTRINGLOG#" ADDNEWLINE="no">
</CFIF>
...
<CFIF GetClientDetails.recordcount gt 0>
	...
	<cfelse>
		...
		<cfelse>
			#COCD# File Generated Not On Client Machine #REPLACE(FileName,'\','\\','ALL')#<br>
		</CFIF>
	</CFIF>
</CFIF>
</CFOUTPUT>
