...
<!--- <CFIF getsystemsettingsvalue.UCCEXPORT_COMPULSORY EQ "Y">	
	<CFINCLUDE TEMPLATE="generateUCICURMCXComp.cfm">
	<cfabort>
</CFIF> --->
...
<CFQUERY NAME="GetClientDetails" datasource="#Client.database#">
	SELECT DISTINCT  A.CLIENT_ID, pan_name CLIENT_NAME,B.MARKET,A.COMPANY_CODE,A.REGISTRATION_DATE,A.LAST_MODIFIED_DATE,
		...
		REL_WITH_INTRODUCER,INTRODUCER_ACC_CODE ,Registration_Number,Registration_Authority,Place_Of_Reg,Convert(Varchar(11),Date_of_Reg,106)  Date_of_Reg,
			...
			A.First_Name,A.Middle_Name,A.Last_Name,UIN,TYPEOFFACILITY,
			ISNULL(PAN_NAME,A.CLIENT_NAME) PAN_NAME,CIN,MOBILE_NO,CLIENT_ID_MAIL,UpdationFlag
...
<!--- 				<CFSET FINALSTRING	   = "#FINALSTRING##INTRODUCER_NAME#,#INTRODUCER_ACC_CODE#,#RELWITHINT#,1,E">
 --->				
			<!--- <cfif "#DateFormat(now(),'yyyymmdd')#" gte '20130111'> --->
				...
				<cfif CATEGORY1 eq "13">
					<CFSET FINALSTRING	   = "#FINALSTRING##LEFT(INTRODUCER_NAME,60)#,#LEFT(INTRODUCER_ACC_CODE,10)#,#LEFT(RELWITHINT,25)#,,,1,#TypeOfFacility#,#LEFT(MOBILE_NO1,20)#,#LEFT(EMAILID1,50)#,,#LEFT(CIN1,25)#,,E">
				<cfelse>
					<CFSET FINALSTRING	   = "#FINALSTRING##LEFT(INTRODUCER_NAME,60)#,#LEFT(INTRODUCER_ACC_CODE,10)#,#LEFT(RELWITHINT,25)#,,,1,#TypeOfFacility#,#LEFT(MOBILE_NO1,20)#,#LEFT(EMAILID1,50)#,,,,E">	
				</cfif>	
