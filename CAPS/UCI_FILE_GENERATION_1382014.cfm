...
function generateReport(form,count,filefor)
	{
		...
		{
			FileGeneration.location.href="generateUCINEW3.cfm?"+commonParam+"&chkval="+chkval+"&batchNo="+document.frmUCI.txtBatchNo.value+"&frdate="+document.frmUCI.txtDate.value+"&todate="+document.frmUCI.txtToDate.value+"&market="+document.frmUCI.cmbmarket.value+"&chkClient="+chkClient+"&BROKERAGE1="+Brokerage2;
		}
		else if (filefor=='AI')
		{
...
<CFOUTPUT>
  ...
  <CFQUERY NAME="GETCLIENTLIST" datasource="#Client.database#" DBTYPE="ODBC">
		...
		SELECT  A.CLIENT_ID,A.CLIENT_NAME,B.MARKET,A.COMPANY_CODE,A.REGISTRATION_DATE,CONVERT(DATETIME, CONVERT(VarChar(10), A.REGISTRATION_DATE, 103), 103),
		A.LAST_MODIFIED_DATE,'Y' TRDFLG ,ISNULL(PANNO,'')PANNO,''active_inactive
		...
		<cfelse>
			<CFIF IsDefined('txtDate') And Trim(txtDate) is not "" and IsDefined('txtToDate') And Trim(txtToDate) is not "">
				AND A.CLIENT_ID IN (SELECT CLIENT_ID
									FROM AUDITEDDATA
									WHERE
										COLUMN_NAME IN ('RESI_ADDRESS','CITY','STATE','COUNTRY','PINCODE','RESI_TEL_NO','MOBILE_NO','TypeOfFacility','MasterPan','RelationShip','UpdationFlag','CIN')
		...
		,'N' TRDFLG ,ISNULL(PANNO,'')PANNO,''active_inactive
		FROM CLIENT_MASTER A LEFT OUTER JOIN CLIENT_DETAIL_VIEW B
		ON  A.CLIENT_ID=B.CLIENT_ID
		AND A.COMPANY_CODE=B.COMPANY_CODE
		AND B.EXCHANGE='NSE'
...
<!--- 					AND (
 ...
 --->				</CFIF>	
  ...
  <cfelse>
  <CFQUERY NAME="GETCLIENTLIST" datasource="#Client.database#" DBTYPE="ODBC">
 		Select DISTINCT A.company_code,a.client_id,client_name,CLIENT_NATURE,REGISTRATION_DATE,Last_Modified_date,active_inactive,
		'' PANNO,	'n' Trdflg
...
<!--- <CFIF IsDefined('btnView')>		
  ...
  <CFQUERY NAME="GETCLIENTLIST" datasource="#Client.database#" DBTYPE="ODBC">
 		<!--- <CFIF IsDefined('chkTrdClient')> --->
		SELECT  A.CLIENT_ID,A.CLIENT_NAME,B.MARKET,A.COMPANY_CODE,A.REGISTRATION_DATE,CONVERT(DATETIME, CONVERT(VarChar(10), A.REGISTRATION_DATE, 103), 103),
		A.LAST_MODIFIED_DATE,'Y' TRDFLG ,ISNULL(PANNO,'')PANNO
		FROM CLIENT_MASTER A LEFT OUTER JOIN CLIENT_DETAIL_VIEW B
		ON  A.CLIENT_ID=B.CLIENT_ID
		AND A.COMPANY_CODE=B.COMPANY_CODE
		AND B.EXCHANGE='NSE'
...
<INPUT TYPE="TEXT" class="fieldbuttonblue" NAME="txtBatchNo" maxlength="3" SIZE="3" <CFIF IsDefined('txtBatchNo') and txtBatchNo neq ''> VALUE="#txtBatchNo#" <CFELSE> VALUE="01"</CFIF>CLASS="StyleTextBox">
		...
		<TH nowrap>
			...
			<SELECT NAME="cmbmarket" class="fieldbuttonblue" CLASS="StyleTextBox">
				...
				<OPTION VALUE="" <CFIF IsDefined('cmbmarket') and Trim(cmbmarket) eq ''>SELECTED</CFIF> > Cash & FNO</OPTION>
			</SELECT>
		</TH>
		<TH nowrap>
			Cl. Opt. :
  ...
  <TABLE  bgcolor="##ECFFFF"  ID="tabela" WIDTH="100%" BORDER="1px" ALIGN="LEFT"  CELLPADDING="0" CELLSPACING="0">
	<THEAD>
		...
		<cfif panno neq ''>
			<TR STYLE="cursor:pointer; " style="font-size:9px;color:666666;font:"Trebuchet MS";" NOWRAP title="Client's pan no is debarred by exchange">
				<TH>
