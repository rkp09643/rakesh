...
<CFFORM ACTION="IMPORT_MASTER_DBF.cfm" METHOD="POST" ENABLECAB="Yes">
  ...
  <CFIF IsDefined("cmdImportMaster")>
  	<CFOUTPUT>
		...
		<CFSET Market	=	GetExchange.Market>
			...
			<CFQUERY NAME="GetAddress" DBTYPE="Dynamic" >
				SELECT		* 						
  ...
  <CFIF IsDefined("cmdImportBrokerage")>
	<CFQUERY NAME="GetBrk" DBTYPE="Dynamic">
		SELECT		* 						
	...
	<CFOUTPUT QUERY="GetBrk">
		...
		<CFQUERY NAME="GetSlab" DBTYPE="Dynamic">
			SELECT		* 						
			FROM		C:\DBFData\BrkOff.dbf
			WHERE		
					SBCURTP	IN ('1','2','5')
  	...
  	<CFIF	IsDefined("cmdImportClientBrk")>
		...
		<CFQUERY NAME="DeleteBrk" datasource="#Client.database#" DBTYPE="ODBC">
			Delete	BROKERAGE_APPLY
			Where
					COMPANY_CODE 	=	'#UCase(Trim(CmbCoList))#'
		</CFQUERY>
		
		<CFOUTPUT QUERY="GetBrk">
			<CFSET		ModuleDesc		=	"#Mid(MSREFSW, 3, 2)#">
			<CFSET 		FamilyGroup		=	"#Trim(Right(Trim(MSPlace), 5))#">
			
			<CFIF Left(FamilyGroup, 1) is "P" Or 
				  Left(FamilyGroup, 1) is "N" Or 
				  Left(FamilyGroup, 1) is "-" Or 
				  IsNumeric(Left(FamilyGroup, 1))>
		
				<CFSET	FamilyGroup = "">
				  
		  </CFIF>
			
		  <CFIF FamilyGroup is Not "">
		  	
			<CFQUERY NAME="GetFamName" DBTYPE="Dynamic">
				Select 	MSAcName
				From
						C:\DBFData\CBrkCl.dbf
				Where		
						MSAccode	=	'#FamilyGroup#'
			</CFQUERY>
			
			<CFSET	FamilyName	=	FamilyGroup>
			
			<CFIF	GetFamName.RecordCount GT 0>
				<CFSET	FamilyName	=	"#GetFamName.MSAcName#">
			</CFIF>
	
			<CFTRY>
			<cfstoredproc datasource="#Client.database#" procedure="FA_insertFamilyMaster" returnCOde="Yes" >
				<cfprocparam type="In" cfsqltype="CF_SQL_VARCHAR" dbvarname="@COcd_1"  value="#UCASE(CmbCoList)#">
				<cfprocparam type="In" cfsqltype="CF_SQL_VARCHAR" dbvarname="@GroupCode_2" value="#UCASE(FamilyGroup)#">
				<cfprocparam type="In" cfsqltype="CF_SQL_VARCHAR" dbvarname="@GroupName_3" value="#UCASE(FamilyName)#">
				<cfprocparam type="In" cfsqltype="CF_SQL_NUMERIC" dbvarname="@FinStyr_4"  value="2004">
				<cfprocparam type="In" cfsqltype="CF_SQL_NUMERIC" dbvarname="@FinEndyr_5" value="2005">
			</cfstoredproc>
			<CFCATCH TYPE="Database">
			...
			<CFSTOREDPROC Procedure="FACLIENT_CREATION" datasource="#Client.database#" ReturnCode="Yes">
				...
				<CFPROCPARAM type="In" cfsqltype="CF_SQL_VARCHAR" dbvarname="@COCD" value="#UCase(Trim(CmbCoList))#" maxlength="10" null="No">
				<CFPROCPARAM type="In" cfsqltype="CF_SQL_VARCHAR" dbvarname="@CLIENTID" value="#UCase(Trim(FamilyGroup))#" maxlength="20" null="No">
				<CFPROCPARAM type="In" cfsqltype="CF_SQL_VARCHAR" dbvarname="@CLIENTNAME" value="#UCase(Trim(FamilyName))#" maxlength="100" null="No">
				<CFPROCPARAM type="In" cfsqltype="CF_SQL_VARCHAR" dbvarname="@FA_COCD" value="#UCase(Trim(CmbCoList))#" maxlength="10" null="No">
				<CFPROCPARAM type="In" cfsqltype="CF_SQL_INTEGER" dbvarname="@FINSTART" value="2004" maxlength="4" null="No"> 
				<CFPROCPARAM type="In" cfsqltype="CF_SQL_INTEGER" dbvarname="@FINEND" value="2005" maxlength="4" null="No"> 
			</CFSTOREDPROC>
					
			<CFQUERY Name="DeleteClient" datasource="#Client.database#" DBType="ODBC">
				...
				Where
						...
						(
							...
							From	COMPANY_MASTER
							Where
										FA_POSTING		=	'#Trim(CmbCoList)#'
						)
						And	CLIENT_ID		=	'#Ucase(Trim(FamilyGroup))#'
			</CFQUERY>

			<CFQUERY Name="DeleteClientDetails" datasource="#Client.database#" DBType="ODBC">
				Delete	CLIENT_DETAILS
				Where	CLIENT_ID	=	'#Ucase(Trim(FamilyGroup))#'
			</CFQUERY>
			
			<CFQUERY Name="UpdateDetails" datasource="#Client.database#" DBType="ODBC">
				Update	Client_Master
				Set
						Family_Group	=	'#Ucase(Trim(FamilyGroup))#'
				Where	CLIENT_ID		=	'#Ucase(Trim(MSAccode))#'
			</CFQUERY>
			...
			<CFQUERY Name="UpdateDetails" datasource="#Client.database#" DBType="ODBC">
				...
				Where	
						SUBLEDGERACCOUNTCODE	=	'#Ucase(Trim(MSAccode))#'
			</CFQUERY>
		</CFIF>
		...
		<CFTRY>
			  ...
			  And	ValidateModule.RecordCount GT 0>
	 			...
	 			<CFQUERY Name="AddNewClientBrkApply" datasource="#Client.database#" DBType="ODBC">
					...
					(
							...
							'#UCase(Trim(Module_No))#', 1, 1, 'Y', 'Y', 'Y', 'Y'
					)
				</CFQUERY>
			</CFIF>
			...
			<cfcatch TYPE="Database">
  			...
  			<CFQUERY NAME="InsertBlank" datasource="#Client.database#" DBTYPE="ODBC">
				...
				Select 
						'#UCase(Trim(CmbCoList))#', Client_ID, '2004-10-01', Null, Null, Null,
						 1, 1, 'Y', 'Y', 'Y', 'Y'
				From
						Client_Master
				Where
