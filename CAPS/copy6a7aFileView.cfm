...
<!--- <CFTRY> --->
	<CFQUERY NAME="GetOrderTrades" datasource="#Client.database#">
		SELECT	
				...
				SUM( QUANTITY * PRICE_PREMIUM ) / SUM(QUANTITY)Rate,
				...
				SUM( QUANTITY * PRICE_PREMIUM ) / SUM(QUANTITY)Rate,
				CASE WHEN A.BUY_SALE = 'SALE' THEN 
					SUM(QUANTITY * (PRICE_PREMIUM-(TRADE_BROKERAGE+DELIVERY_BROKERAGE))) / SUM(QUANTITY)
				Else 
					SUM(QUANTITY * (PRICE_PREMIUM+(TRADE_BROKERAGE+DELIVERY_BROKERAGE)))  / SUM(QUANTITY)
				END NetRate
		FROM	
...
<DIV align="center" id="TableData" style="top:11%; ">
	...
	<CFLOOP query="GetOrderTrades">
		...
		<cfloop query="Others">
			...
			<CFIF	GetSttApplied.RecordCount	EQ	0>
			...
			<CFSET	QTY		=	Quantity>
			<CFIF	DelAdjCode	EQ	13 And RATE GT 0>
