...
<CFFORM ACTION="IMPORT_MASTER_DBF10.cfm" METHOD="POST" ENABLECAB="Yes">
  ...
  >>	File Name for "Client Brokerage Relation" is "CBrkCl.dbf" (Comtek File Name : AMSTAE04.DBF).
  >>	FOR BROKERAGE MASTER
  			FOR BSE
				1.) Y IS N
				2.) O IS T	
				3.) D IS A
...
<BR><BR>
  ...
  <CFIF IsDefined("cmdImportMaster")>
  	<CFOUTPUT>
		...
		<CFSET Market	=	GetExchange.Market>
			...
			<CFQUERY NAME="GetAddress" DATASOURCE="DBFData" DBTYPE="ODBC">
				SELECT		* 						
				FROM		CapsAdd.dbf
  ...
  <CFIF IsDefined("cmdImportBrokerage")>
	<CFQUERY NAME="GetBrk" DATASOURCE="DBFData">
		SELECT		* 						
		FROM		BrkOff.dbf
		WHERE		
					SBCURTP	IN ('1','2','5')
	...
	<cfif Len(Trim(SBBRKSLB)) gt 0>
		...
		<CFQUERY NAME="GetSlab" DATASOURCE="DBFData">
			SELECT		* 						
			FROM		BrkOff.dbf
			WHERE		
					SBCURTP	IN ('1','2','5')
  	...
  	<CFIF	IsDefined("cmdImportClientBrk")>
		<CFQUERY NAME="GetBrk" DATASOURCE="DBFdata" DBTYPE="ODBC">
			SELECT		* 						
			FROM		CBrkCl
		</CFQUERY>		
	...
	<CFOUTPUT QUERY="GetBrk">
		...
		<CFTRY>
			...
			<CFSET		ModuleDesc		=	"#Mid(MEPLACE, 3, 1)##Mid(MEREFSW, 2, 1)#">
			...
			<CFIF	ValidateClient.RecordCount GT 0
			  And	ValidateModule.RecordCount GT 0>
	 			...
	 			<CFQUERY Name="AddNewClientBrkApply" datasource="#Client.database#" DBType="ODBC">
					...
					Values
					(
							'#UCase(Trim(CmbCoList))#', '#UCase(Trim(MEACCODE))#', '2004-10-01', Null, Null,
	  ...
	  <CFQUERY NAME="InsertBlank" datasource="#Client.database#" DBTYPE="ODBC">
			...
			Select 
					'#UCase(Trim(CmbCoList))#', Client_ID, '2004-10-01', Null, Null, 0, 3, 3,
					'Y','Y','Y'
			From
					Client_Master
			Where
