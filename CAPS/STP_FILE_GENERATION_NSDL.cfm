...
<CFSET Trade_Date = "#txtOrderDate#"> --->
	...
	<CFSET	OnlyFileName	=	"#LEFT(cocd,1)#_#SETTLEMENT_NO#_#txtClientID#_#SCRIP_SYMBOL_ORDER_NO#.txt">
...
<!--- 					<Cfset TRADE_NO  = TRADE_Number>
 ...
 ---> 			
 				<cfloop query="CONTRACTDETAILQUERY" startrow="1" endrow="1">
					...
					<Cfset TRADE_TIME  = TimeFormat(TRADE_dateTIME,'hhmmss')>
					<!--- ***************** --->
					<CFFILE ACTION="APPEND" FILE="#File_Name#" OUTPUT="#fileConvertString(BatchNumber, 7, 'R', '0')#13#fileConvertString(LineNumber, 5, 'R', '0')##fileConvertString('', 2, 'L', ' ')##fileConvertString(Broker_Sebi_RegnNo, 12, 'L', ' ')##fileConvertString(CNNO, 16, 'L', ' ')##fileConvertString(ORDER_NO, 16, 'R', '0')##fileConvertString(Trade_No, 16, 'R', '0')##fileConvertString(Trade_Time, 6, 'R', '0')##fileConvertString(DETAILQTY, 15, 'R', '0')##fileConvertString(RATE7, 15, 'R', '0')#" ADDNEWLINE="Yes">
 				</cfloop>		
 			<cfelse>
				<cfloop query="CONTRACTDETAILQUERY">
					...
					<Cfset TRADE_TIME  = TimeFormat(TRADE_dateTIME,'hhmmss')>
					<!--- ***************** --->
					<CFFILE ACTION="APPEND" FILE="#File_Name#" OUTPUT="#fileConvertString(BatchNumber, 7, 'R', '0')#13#fileConvertString(LineNumber, 5, 'R', '0')##fileConvertString('', 2, 'L', ' ')##fileConvertString(Broker_Sebi_RegnNo, 12, 'L', ' ')##fileConvertString(CNNO, 16, 'L', ' ')##fileConvertString(ORDER_NO, 16, 'R', '0')##fileConvertString(Trade_No, 15, 'R', '0')##fileConvertString(Trade_Time, 6, 'R', '0')##fileConvertString(DETAILQTY, 15, 'R', '0')##fileConvertString(RATE7, 15, 'R', '0')#" ADDNEWLINE="Yes">
 				</cfloop>		
			</cfif>	
		</CFLOOP>
	</CFLOOP>	
