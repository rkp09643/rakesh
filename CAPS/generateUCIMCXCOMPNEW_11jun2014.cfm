...
<cfquery name="GetName" datasource="#Client.database#">
	select ltrim(rtrim(CHEKERUSERLIST)) CHEKERUSERLIST,FileVersion
...
<CFQUERY NAME="GetClientDetails" datasource="#Client.database#">
	SELECT DISTINCT  A.CLIENT_ID, pan_name CLIENT_NAME,B.MARKET,A.COMPANY_CODE,A.REGISTRATION_DATE,A.LAST_MODIFIED_DATE,
		...
		REL_WITH_INTRODUCER,INTRODUCER_ACC_CODE ,Registration_Number,Registration_Authority,Place_Of_Reg,Convert(Varchar(11),Date_of_Reg,106)  Date_of_Reg,
			...
			ISNULL(PAN_NAME,A.CLIENT_NAME) PAN_NAME,TypeOfFacility,CLIENT_ID_MAIL EMAILID,MOBILE_NO
...
<CFLOOP QUERY="getClientDetails">
	...
	<CFIF  len(GetClientDetails.PAN_NO) eq 10>				
				...
				<CFSET FINALSTRING = "#FINALSTRING##UCASE(CLIENTID)#,#UCASE(FIRSTNAME)#,#UCASE(MIDDLENAME)#,#UCASE(LASTNAME)#,#UCASE(CATEGORY1)#,#LEFT(TRIM(REPLACE(UCASE(Add1),',',' ','ALL')),75)#,#LEFT(TRIM(REPLACE(UCASE(Add2),',',' ','ALL')),75)#,#LEFT(TRIM(REPLACE(UCASE(Add3),',',' ','ALL')),75)#,#UCASE(R_CITY)#,#UCASE(R_STATE)#,#UCASE(R_COUNTRY)#,#UCASE(PIN_CODE)#,,#AGREEMENT_DATE1#,#BIRTHDATE#,#UCASE(PAN_NO1)#,,#UCASE(UIN)#,">
				...
				<CFIF COCD EQ "MCX">
					<cfif "#DateFormat(now(),'yyyymmdd')#" gte '20130201'>
						<CFSET FINALSTRING	   = "#FINALSTRING#,,,N,,1,#TypeOfFacility1#,#MOBILE_NO1#,#EMAILID1#,,,,E">	
					<CFELSE>
						<CFSET FINALSTRING	   = "#FINALSTRING#,,,N,,1,#TypeOfFacility#,#MOBILE_NO1#,#EMAILID1#,,E">	
					</cfif>	
				<cfelse>
					<CFSET FINALSTRING	   = "#FINALSTRING#,,,N,,1,E">
				</CFIF>
